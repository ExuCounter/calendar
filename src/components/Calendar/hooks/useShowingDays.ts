...
const getPreviousMonthDays = ({ showingYear, showingMonth }: { showingYear: number; showingMonth: number }) => {
  const { countOfDays, startLocalDay } = useMonth(showingMonth - 1, showingYear)

  return new Array(startLocalDay).fill(null).map((_, idx) => {
    return { day: countOfDays - startLocalDay + idx + 1, position: "previous" as DayPosition }
  })
}

const getCurrentMonthDays = ({ showingYear, showingMonth }: { showingYear: number; showingMonth: number }) => {
...
export const useShowingDays = ({ showingYear, showingMonth }: { showingYear: number; showingMonth: number }) => {
